...
@city
def theodora(files_in, file_out, compression, event_range, print_mod, detector_db, run_number,
             drift_v,
             s1_nmin, s1_nmax, s1_emin, s1_emax, s1_wmin, s1_wmax, s1_hmin, s1_hmax, s1_ethr,
             s2_nmin, s2_nmax, s2_emin, s2_emax, s2_wmin, s2_wmax, s2_hmin, s2_hmax, s2_ethr, s2_nsipmmin, s2_nsipmmax, ### SiPM patch to re-use IC code
    ...
    with tb.open_file(file_out, "w", filters = tbl.filters(compression)) as h5out:
        ...
        return fl.push(source = pmap_from_files(files_in),
                    pipe   = fl.pipe(
