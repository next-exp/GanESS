...
n_baseline = 225
timebin    = 8
n_s1       = int(30 * 1000 / timebin)
n_mid      = int(24 * 1000 / timebin)
slice_mus  = 5
n_interval = int(slice_mus  * 1000 / timebin)
n_maw      = 250
thr_maw    = 0.2
...
def generate_wvf_dataset(infile, outfile):
    rwf_to_cwf      = calibrate_pmts_wf_data("gap", run_number, n_baseline, n_maw, thr_maw)
    ...
    with tb.open_file(infile, 'r') as h5in:
        ...
        ntot      = all_wvfs.shape[2]
        nslices   = np.ceil(ntot/n_interval).astype(int)
...
#        d_all_h      = d_cwvfs_sum.max(axis=1)
        ...
        for i in range(nslices):
            slices_info[f'energy_t{i*slice_mus}_t{(i+1)*slice_mus}'] = cwvfs_sum[:,    i*n_interval:(i+1)*n_interval].sum(axis=1)
...
data_pmt = db.DataPMT('gap', run_number)
...
if fpmaps_dst:
    pmaps_dir = os.path.dirname(infiles[0])
    origin      = 'octavia'
