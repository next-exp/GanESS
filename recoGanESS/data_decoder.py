...
parser.add_argument('--reprocess', '-rp',  help="Reprocess run"   , action='store_true')
...
infiles     = args.input #glob.glob(args.input)
...
if os.path.isdir(new_dir):
    if freprocess:
        print(f"Reprocessing run {run_number}, all files will be deleted, press Enter to continue")
        input()
        os.system(f"rm -rf {new_dir}")
    else:
        sys.exit(f'Trying to overwrite already processed run, aborting.')
os.system(f"mkdir {new_dir}") 

new_dir  += 'raw/'
os.system(f"mkdir {new_dir}") 

#new_path = new_dir + os.path.basename(inpath)
#os.system(f"mv {inpath} {new_path}")
...
for i, infile in enumerate(infiles):
    ...
    outfile = new_dir + os.path.basename(infile).replace('.txt', '.h5')
