...
REGISTER_CLASS(GaP7, GeometryBase)
...
{
  // Messenger
  msg_ = new G4GenericMessenger(this, "/Geometry/GaP7/",
                                "Control commands of the GaP7 geometry.");
...
G4ThreeVector GaP7::GenerateVertex(const G4String& region) const
{
  ...
  //// Gas regions
    ...
    G4Exception("[GaP7]", "GenerateVertex()", FatalException,
...
G4ThreeVector GaP7::GenerateVertexGas(const G4String& region) const
{
    G4ThreeVector vertex;
    ...
    else{G4Exception("[GaP7]", "GenerateVertex()", FatalException,
                "Unknown vertex generation region!");}
    return vertex;
}


void GaP7::DefineConfigurationParameters()
{
  // Gas pressure
...
void GaP7::BuildTPC(G4Material* gas, G4Material* cath_mat, G4Material* gate_mat, G4Material* anode_mat, G4LogicalVolume* logic_vessel_steel)
{
    //Gas
    ...
    IonizationSD* active_sd = new IonizationSD("/GaP7/DRIFT");
    ...
    G4ThreeVector pmt1_Ps = G4ThreeVector(-15.573*mm,-32.871*mm,pmt_z);
    G4ThreeVector pmt2_Ps = G4ThreeVector(20.68*mm,-29.922*mm,pmt_z);
    G4ThreeVector pmt3_Ps = G4ThreeVector(-36.253*mm,-2.949*mm,pmt_z);
    G4ThreeVector pmt4_Ps = G4ThreeVector(0,0,pmt_z);
    G4ThreeVector pmt5_Ps = G4ThreeVector(36.253*mm,2.949*mm,pmt_z);
    G4ThreeVector pmt6_Ps = G4ThreeVector(-20.68*mm,29.922*mm,pmt_z);
    G4ThreeVector pmt7_Ps = G4ThreeVector(15.573*mm,32.871*mm,pmt_z);
